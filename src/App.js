...
function App() {
  ...
  return (
    <div className="App">
      <TableWithBrowserPagination className="table table-striped"  data={data} variant="listview" pageSize={4}>
        <Column header="Clave" field={data.map(element=>(element.id))}/>
        <Column header="Foto" field="foto"/>
        ...
        <tbody>
          {data.map(element=>(
            <tr>
              ...
              <td><button className="btn btn-primary" onClick={()=>seleccionarEmp(element, 'Editar')}>Modificar</button></td>{"   "} 
